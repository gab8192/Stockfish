...
namespace NNUE {
  ...
  bool needRefresh(Color side, Square oldKing, Square newKing) {
    const bool oldMirrored = file_of(oldKing) >= FILE_E;
    const bool newMirrored = file_of(newKing) >= FILE_E;
  ...
  void Accumulator::refresh(Position& pos, Color side) {
    ...
    while (occupied) {
      ...
      addPiece(kingSq, side, pos.piece_on(sq), sq);
    }
    updated[side] = true;
  }
  ...
  Value evaluate(Position& pos, Accumulator& accumulator) {

    constexpr int divisor = (32 + OutputBuckets - 1) / OutputBuckets;
    int outputBucket = (popcount(pos.pieces()) - 2) / divisor;
    ...
    {
      Vec* acc = (Vec*) accumulator.colors[pos.side_to_move() ^ them];
    ...
    return Value((unsquared * NetworkScale) / NetworkQAB);
  }
