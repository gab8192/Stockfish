...
constexpr int futility_move_count(bool improving, bool ttm, Depth depth) {
    return improving ? (3 + depth * depth - ttm) 
                     : (3 + depth * depth - ttm) / 2;
}
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (!moveCountPruning)
                moveCountPruning = moveCount >= futility_move_count(improving, ttMove.is_ok(), depth);
